...
/**
 * KAT 本部の役員4名の社員コード（明石側では給与計算・給与画面・社労士出力の
 * 対象外にする。KAT WORLD 側で処理される）。
 * ※ requires_punch=false の従業員（例：産休中の DA037）とは別扱いにする必要が
 *    あるが、両者を DB 上のフラグ1つで区別する方法がまだ確定していないため、
 *    区別する項目が決まるまで社員コード直書きの暫定運用とする。
 *    区別項目が決まり次第、この定数と各画面/計算側の参照を置き換える。
 * ※ 使用箇所は必ずこの定数を import すること（複数箇所に同名の定数を作らない）。
 */
export const HONBU_EMPLOYEE_CODES = ["D02", "D18", "D49", "D67"] as const;
