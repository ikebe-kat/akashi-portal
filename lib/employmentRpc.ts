...
// 従業員の期間内ステータス（active/not_employed/excluded）を全員分Mapで取得
// 判定ロジックはDB関数 fn_employment_status_in_period に一本化（3社共通）
...
// 期間内の休職日を (employee_id, 日付) Setで取得
// 判定ロジックはDB関数 fn_leave_days_in_period に一本化（3社共通）
