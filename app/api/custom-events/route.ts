...
const HISTORY_COLUMNS = [
  ...
  "actor_name", "actor_employee_id", "event_title", "event_date", "operated_at",
];
...
export async function POST(req: NextRequest) {
  try {
    ...
    switch (body.action) {
      ...
      case "list_histories": {
        let q: any = supabaseServer.from("custom_event_histories").select(body.select || "*");
        q = applyFilters(q, body.filters);
        q = applyOrder(q, body.order);
        if (body.limit) q = q.limit(body.limit);
        const { data, error } = await q;
      ...
      case "insert_history": {
        const payload = sanitize(HISTORY_COLUMNS, body.data);
        const { error } = await supabaseServer.from("custom_event_histories").insert(payload);
        return NextResponse.json({ data: null, error: error?.message || null });
      }
