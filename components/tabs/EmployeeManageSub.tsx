...
const LeavePanel = ({ empId, onMsg }: { empId: string; onMsg: (m: string) => void }) => {
  ...
  return (
    <div>
      ...
      {leaves.map(lv => (
        <div key={lv.id} style={{ border: `1px solid ${T.border}`, borderRadius: 3, padding: 14, marginBottom: 8, backgroundColor: "#fff" }}>
          <div style={{ display: "flex", justifyContent: "space-between", alignItems: "center", marginBottom: 6 }}>
            <div>
            ...
            <div style={{ display: "flex", gap: 4 }}>
              ...
              <button onClick={() => handleDelete(lv)} style={{ padding: "4px 8px", borderRadius: 3, border: `1px solid ${T.danger}`, backgroundColor: "#fff", color: T.danger, fontSize: 11, cursor: "pointer" }}>削除</button>
            </div>
          </div>
          <div style={{ fontSize: 12, color: T.textSec }}>
            <span style={{ padding: "1px 6px", borderRadius: 3, backgroundColor: T.primary + "12", color: T.primary, fontSize: 11, fontWeight: 600, marginRight: 8 }}>{lv.leave_type}</span>
            {lv.exclusions.length > 0 ? lv.exclusions.map(e => <span key={e} style={{ padding: "1px 5px", borderRadius: 3, backgroundColor: T.bg, color: T.textSec, fontSize: 10, marginRight: 4 }}>{exclusionLabel(e)}</span>) : <span style={{ fontSize: 11, color: T.textMuted }}>除外面なし</span>}
          </div>
        </div>
      ))}
      {showForm && (
        <div style={{ border: `1px solid ${T.primary}40`, borderRadius: 3, padding: 16, marginTop: 8, backgroundColor: "#FAFCFF" }}>
          ...
          <div style={{ display: "flex", gap: 8 }}>
            <button onClick={() => setShowForm(false)} style={{ padding: "8px 16px", borderRadius: 3, border: `1px solid ${T.border}`, backgroundColor: "#fff", color: T.textSec, fontSize: 12, cursor: "pointer" }}>キャンセル</button>
            <button onClick={handleSave} disabled={saving} style={{ padding: "8px 16px", borderRadius: 3, border: "none", backgroundColor: T.primary, color: "#fff", fontSize: 12, fontWeight: 600, cursor: saving ? "default" : "pointer", opacity: saving ? 0.6 : 1 }}>{saving ? "保存中..." : editLeave ? "更新" : "追加"}</button>
          </div>
        </div>
      )}
    </div>
      ...
      <div style={{ backgroundColor: "#fff", borderRadius: "12px 12px 0 0", padding: "20px", width: "100%", maxWidth: 560, maxHeight: "85vh", overflowY: "auto", animation: "slideUp 0.3s ease" }} onClick={e => e.stopPropagation()}>
        ...
        <div style={{ display: "flex", gap: 4, marginBottom: 16, borderBottom: `1px solid ${T.border}` }}>
          {(["dependents", "documents", "nyusha", "leaves"] as const).map(t => (
            <button key={t} onClick={() => setTab(t)} style={{ padding: "10px 16px", border: "none", backgroundColor: "transparent", cursor: "pointer", fontSize: 13, fontWeight: tab === t ? 700 : 400, color: tab === t ? T.primary : T.textSec, borderBottom: tab === t ? `3px solid ${T.primary}` : "3px solid transparent" }}>{t === "dependents" ? "扶養家族" : t === "documents" ? "個人書類" : t === "nyusha" ? "入社シート" : "休職管理"}</button>
          ))}
        </div>
